...
## 2026-08-27 — Lighter models execute routine implementation by default
**Proposed and approved by:** Mehul (binding instruction: "use lighter models to do the manual coding and stuff"). **Agents:** Claude and Codex must both follow it.
**What:** Every non-trivial implementation follows **heavy spec → light implementation → heavy verification**. Lighter models receive bounded, written tasks for mechanical coding, test scaffolds, adapter/UI wiring from an agreed contract, formatting, and batch-job operation. Heavy models retain architecture, eval/metric correctness, preprocessing fidelity, contamination/data-split decisions, thresholds/calibration, licensing, gate verdicts, and final diff review. Heavy direct implementation is the exception; if used, its reason is recorded in the owning CHANGELOG.
**Evidence:** Mehul's explicit 2026-08-27 instruction; `CLAUDE.md`/`AGENTS.md` existing model-economy rules; `coordination/PLAN-UPDATE-2026-08-27.md`.
**Reverses if:** Mehul changes the delegation policy, or a specific task cannot be safely decomposed without transferring judgment-heavy context (record the exception rather than silently ignoring the rule).

## 2026-08-27 — Post-LOTA execution delta: quality-conditioned correction, bounded heavy rescue, DegradePrint parked
**Proposed by:** Claude (A-023; measured diagnostic and model-availability audit). **Approved with scientific-boundary corrections by:** Codex (B-020), supported by independent Claude audit A-024. **Mehul:** binding request to jointly replan from `docs/techjam_track5_update/`.
**What:** Preserve the working CF-384 baseline and original phase history. The Phase-2 common path now tests a **quality-conditioned correction head** over the primary logit; convex multi-expert fusion is active only if a second always-on expert earns its slot. PGC is the first licensed heavy preflight candidate but stays outside the 15k fitting cache and may become selective rescue only after licence, parity, parameter, memory, latency and complementarity gates. GAPL code integration is blocked while its official GitHub repo lacks a licence, despite the HF model card's MIT tag. LOTA reproduction is removed from the deadline path. New DegradePrint logit-response/embedding work is removed from the critical path: logit response is **parked**, while embedding drift is untested/deferred. No embeddings, blur probe, or heavy expert enter the next cache generation. Existing probes remain provisional pending a protected reliability ablation. The long cache cannot launch until B-018 is repaired/re-reviewed and the balanced manifests, sealed denylist, pHash separation, protected pilot, and feature set pass.
**Evidence boundary:** the 24k-row unprotected pilot diagnostic measured mean worst-family fake recall 0.211 (primary), 0.604 (+quality), 0.612 (+quality+response), and 0.254 (+response), with response-over-quality only +0.8 point and unstable sign. Because that cache is `UNPROTECTED_SMOKE_ONLY`, obsolete v1, lacks an untouched/generator-held-out test and paired bootstrap, it prioritizes work but is not a submission result, final kill, or fallback trained model. The corrected operational plan is `coordination/PLAN-UPDATE-2026-08-27.md`; historical `06-build-plan.md` remains untouched.
**Evidence:** `handoffs/2026-08-27_post-lota-replan.md`; `scripts/diagnostics/degradeprint_probe.py`; A-023/A-024/B-020; official [PGC repository](https://github.com/xiaoyu6868/PGC), [PGC weights](https://huggingface.co/xiaoyuzhou68/PGC_ckpt), [GAPL repository](https://github.com/UltraCapture/GAPL), and [GAPL weights](https://huggingface.co/AbyssLumine/GAPL).
**Reverses if:** protected, source-disjoint paired evidence shows response/embeddings add >=2 points or a clear selective-risk/flip gain under clean constraints; measured heavy-expert throughput fits <=12 h and complementarity earns always-on use; GAPL repository licensing becomes explicit; or Mehul changes the schedule/scope.
