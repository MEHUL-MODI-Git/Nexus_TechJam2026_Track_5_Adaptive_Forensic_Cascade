...
line 118 requires `scripts/run_eval.py --config configs/frozen.yaml` to reproduce every reported
table. There was no `configs/frozen.yaml` and `run_eval.py` had no `--config`; the command exits
with an argparse error. README §5 documented the old 8,000-row placeholder smoke diagnostic, not
the protected tables actually published. Codex ran the literal command and it failed.

Now built: `configs/frozen.yaml` (generated by `scripts/make_frozen_config.py`) records, for each
of the **10 published tables**, the artifact and its SHA-256, the inputs it was computed from and
theirs, and the command that regenerates it. `scripts/reproduce_frozen.py` verifies all of it and
`run_eval.py --config` delegates to it. Current state: **10 verified, 0 drifted, 0 missing.**

Two design points worth keeping:
- **Inputs are checked, not just artifacts.** An artifact that still matches while its inputs moved
  is worse than a mismatch, because it looks like agreement.
- **The sealed entry is `summary_only` and the verifier refuses any sealed entry that is not.** The
  sealed subset is scored exactly once and already was; a reproduction tool that could re-run
  inference on it would be a defect, not a feature.

**The one-shot internal-test reporter emitted NaN and returned 0.** Codex copied the real complete
manifest over a cache of 39 rows / 2 sources with a missing condition; the script checked only
`manifest.role`, hashed the manifest but never the rows, and wrote NaN headline statistics with
`json.dumps`'s default `allow_nan=True` — which is not even valid JSON. It now validates manifest
counts against the rows present, exactly one of every official condition per source, consistent
labels and split, finite in-range expert scores, the frozen expert revision, and refuses any
non-finite value in its output. The document now records `cache_rows_sha256`, because hashing a
manifest proves nothing about the rows it claims to describe.

**Re-run reproduces every published number exactly** — worst-family 0.8258, clean FPR 0.0833,
accuracy 0.9090, +0.4916 CI95 [+0.4753, +0.5083]. Note the re-run used the shipped
`router_reliability.pt` where the original used stage-1 `router.pt`: identical to the last decimal,
which independently confirms the stage-2 reliability fit left the classifier untouched.

**Ablation provenance and scope.** `ablation_matrix.py` called its thresholds dev-fitted in prose,
comment and committed artifact while fitting them on train — the same mislabelling as the freeze
(R2/S1), in a second file. Corrected without changing a threshold. It also ran 5 rungs while calling
itself the full ladder; `probability_mean` and `fixed_weights` are now included. They are
near-degenerate with one expert, and the refit proves it rather than asserting it: `static_average`,
`probability_mean` and `fixed_weights` return **numerically identical** results to every decimal
(thr 0.12725, dev 0.1849, test 0.1827, clean FPR 0.0127, accuracy 0.8351). With one expert a
logit-space mean, a probability-space mean and a weighted sum over one weight are the same monotone
function of one score, so any threshold-based metric must agree. Published in README §7 with that
explanation. The five previously-reported rungs reproduce byte-identically.
